...
const Checkout = () => {
  ...
  const handleCancel = async () => {
    if (isExpired) return;
    const confirmResult = await showConfirm('Xác nhận hủy', 'Bạn có chắc chắn muốn hủy đơn đặt phòng này? Phòng sẽ không còn được giữ cho bạn nữa.', 'Có, hủy', 'Không');
    ...
    try {
      ...
      const res = await axios.post(`http://localhost:5000/api/bookings/${bookingId}/cancel-pending`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      if (res.data.success) {
        setIsExpired(true);
        setTimeLeft(0);
        await showAlert('Thành công', 'Đã hủy đơn đặt phòng thành công.', 'success');
        navigate('/');
      }
    } catch (err) {
      console.error(err);
      showAlert('Lỗi', err.response?.data?.message || 'Không thể hủy đơn đặt phòng.', 'error');
    }
  };
            ...
            <div className="qr-box glass-panel">
              ...
              {!isExpired && (
                <button 
                  ...
                  style={{ marginTop: '10px', fontWeight: 600, padding: '12px', color: '#dc2626', borderColor: '#fecaca', backgroundColor: '#fef2f2' }} 
