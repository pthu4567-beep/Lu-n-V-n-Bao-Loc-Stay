...
const AdminDashboard = () => {
  ...
  const [newHomestayImageFile, setNewHomestayImageFile] = useState(null);
  ...
  const [newRoomTypeImageFile, setNewRoomTypeImageFile] = useState(null);
  const [newRoomTypeAmenitiesFiles, setNewRoomTypeAmenitiesFiles] = useState([]);
  ...
  const handleVerifyPayment = async (bookingId) => {
    ...
    try {
      ...
      showToast(err.response?.data?.message || err.response?.data?.error || "Lỗi duyệt thanh toán", 'error');
    }
  };

  const handleSendWarningEmail = async (bookingId) => {
    const token = sessionStorage.getItem('token');
    if (!token) return;

    const confirmResult = await showConfirm('Gửi cảnh báo', `Bạn có chắc muốn gửi email cảnh báo hủy đơn cho khách hàng của đơn #${bookingId}?`);
    if (!confirmResult.isConfirmed) return;

    try {
      const res = await axios.post(`http://localhost:5000/api/admin/orders/bookings/${bookingId}/send-warning`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.data.success) {
        showToast(res.data.message, 'success');
      ...
      showToast(err.response?.data?.message || err.response?.data?.error || "Lỗi gửi email", 'error');
    }
  };
  ...
  const submitHomestay = async () => {
    ...
    try {
      ...
      formData.append('name', newHomestay.name);
      formData.append('description', newHomestay.description || '');
      formData.append('facilities_text', newHomestay.facilities_text || '');
      formData.append('address', newHomestay.address || '');
      formData.append('status', newHomestay.status || 'active');
      formData.append('images_text', newHomestay.images_text || '');
      if (newHomestayImageFile) {
        formData.append('image', newHomestayImageFile);
      }

      const res = await axios.post('http://localhost:5000/api/admin/catalog/hotels', formData, {
  ...
  const submitRoomType = async () => {
    ...
    try {
      const formData = new FormData();
      formData.append('hotel_id', newRoomType.hotel_id);
      formData.append('name', newRoomType.name);
      formData.append('base_price', newRoomType.base_price);
      formData.append('capacity', newRoomType.capacity);
      formData.append('room_amenities_text', newRoomType.room_amenities_text || '');
      formData.append('note', newRoomType.note || '');
      formData.append('image_url', newRoomType.image_url || '');
      
      if (newRoomTypeImageFile) {
        formData.append('image', newRoomTypeImageFile);
      }
      if (newRoomTypeAmenitiesFiles && newRoomTypeAmenitiesFiles.length > 0) {
        for (let i = 0; i < newRoomTypeAmenitiesFiles.length; i++) {
          formData.append('amenity_images', newRoomTypeAmenitiesFiles[i]);
        }
      }

      const res = await axios.post('http://localhost:5000/api/admin/catalog/room-types', formData, {
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'multipart/form-data' }
      });
      if (res.data.success) {
        ...
        setNewRoomTypeImageFile(null);
        setNewRoomTypeAmenitiesFiles([]);
        // Tải lại danh sách
                                  ...
                                  <div style={{ textAlign: 'right' }}>
                                    {(payStatus === 'awaiting_confirmation' || payStatus === 'pending_payment') && (
                                      <button
                                        ...
                                        style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', fontWeight: 600, borderRadius: '8px', padding: '6px 12px', boxShadow: '0 2px 4px rgba(37, 99, 235, 0.15)', transition: 'all 0.2s', border: 'none', background: 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)', color: 'white', marginBottom: '6px' }}
      ...
      {selectedOrder && (
        <div className="admin-modal-overlay animate-fade-in" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'fixed', inset: 0, backgroundColor: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(4px)', zIndex: 9999 }} onClick={() => setSelectedOrder(null)}>
          <div className="modal-content" style={{ background: '#ffffff', borderRadius: '20px', width: '100%', maxWidth: '360px', maxHeight: '90vh', display: 'flex', flexDirection: 'column', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            ...
            {/* Modal Footer */}
            <div style={{ padding: '0.75rem 1rem', background: '#f1f5f9', borderTop: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '10px', justifyContent: 'center' }}>
              ...
              {selectedOrder.status !== 'cancelled' && selectedOrder.status !== 'completed' && (
                <button
                  className="btn"
                  style={{ flex: 1, padding: '0.5rem', fontWeight: 600, borderRadius: '8px', background: 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)', color: 'white', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', fontSize: '0.85rem', whiteSpace: 'nowrap' }}
                  onClick={() => {
                    handleSendWarningEmail(selectedOrder.bookingId || selectedOrder.id);
                  }}
                >
                  <Mail size={18} /> Gửi Cảnh Báo
                </button>
              )}
            </div>
          </div>
        ...
        <div className="admin-modal-overlay" onClick={() => setShowHomestayModal(false)}>
          <div className="admin-modal-content glass-panel" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 500 }}>Hình ảnh Homestay</label>
                <input type="file" className="form-control" accept="image/*" style={{ width: '100%' }} onChange={e => setNewHomestayImageFile(e.target.files[0])} />
              </div>
              <div>
        ...
        <div className="admin-modal-overlay" onClick={() => setShowRoomTypeModal(false)}>
          <div className="admin-modal-content glass-panel" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 500 }}>Hình ảnh loại phòng</label>
                <input type="file" className="form-control" accept="image/*" style={{ width: '100%' }} onChange={e => setNewRoomTypeImageFile(e.target.files[0])} />
              </div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '1.5rem' }}>
