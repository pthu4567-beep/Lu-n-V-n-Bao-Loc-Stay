                      ...
                      <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
                          ...
                          <input 
                            ...
                            min="0"
                            max={room.available}
                            value={selectedRooms[room.id] !== undefined ? String(selectedRooms[room.id]) : '0'}
                            onChange={(e) => {
                              if (e.target.value === '') {
                                handleRoomCountChange(room.id, 0, room.available);
                                return;
                              }
                              let val = parseInt(e.target.value, 10);
                              if (isNaN(val)) val = 0;
                              if (val > room.available) val = room.available;
                              if (val < 0) val = 0;
                              handleRoomCountChange(room.id, val, room.available);
                            }}
                            onBlur={(e) => {
                            ...
                            style={{ padding: '10px 5px', fontWeight: 600, width: '50px', textAlign: 'center', border: 'none', background: 'transparent', outline: 'none', margin: 0, appearance: 'none', MozAppearance: 'textfield' }}
                          />
                          <button 
                            style={{ padding: '10px 15px', border: 'none', background: 'transparent', cursor: 'pointer', fontWeight: 600, color: '#64748b' }}
                            onClick={() => handleRoomCountChange(room.id, (selectedRooms[room.id] || 0) + 1, room.available)}
          ...
          <div className="booking-widget glass-panel">
            ...
            <div className="widget-form">
              ...
              <div className="form-group">
                ...
                <div style={{ display: 'flex', alignItems: 'center', background: '#fff', borderRadius: '8px', border: '1px solid var(--primary-200)', overflow: 'hidden', focusWithin: 'border-color: var(--primary-500)' }} className="guest-input-wrapper">
                  <button 
                    type="button"
                    style={{ padding: '10px 15px', border: 'none', background: 'var(--primary-50)', cursor: 'pointer', fontWeight: 600, color: 'var(--primary-700)', borderRight: '1px solid var(--primary-200)' }}
                  ...
                  <input 
                    ...
                    style={{ border: 'none', textAlign: 'center', flex: 1, padding: '10px', outline: 'none', appearance: 'none', MozAppearance: 'textfield', margin: 0 }}
                  />
                  <button 
                    type="button"
                    style={{ padding: '10px 15px', border: 'none', background: 'var(--primary-50)', cursor: 'pointer', fontWeight: 600, color: 'var(--primary-700)', borderLeft: '1px solid var(--primary-200)' }}
                    onClick={() => {
                      let maxVal = totalCapacity > 0 ? totalCapacity : 50;
                      setGuestCount(Math.min(maxVal, guestCount + 1));
                    }}
                    disabled={totalCapacity > 0 && guestCount >= totalCapacity}
                  >
                    +
                  </button>
                </div>
